var generateTree = function(data)
{
    ...
    function update(source) {
        ...
        node.select('text')
            ...
            .html(function(d) {
                return (d.tooltip) ? "<title>"+d.tooltip+"</title>"+d.name : d.name;
            });
...
$(function(){
    ...
    $('.viewJSON').on('click', function(){
        ...
        {
            $('#json-loader').css({left: '-490px'});
            $('.loadStructure').text("Load Structure");

            $('#json-container').css({left: '0'});
            $(this).text("Hide JSON");
        }
        else
        {
            $('#json-container').css({left: '-490px'});
            $(this).text("Show JSON");
        }*/
    });

    /*$('.loadStructure').on('click', function(){
       if($(this).text() === "Load Structure")
       {
            $('#json-container').css({left: '-490px'});
            $('.viewJSON').text("Show JSON");

            $('#json-loader').css({left: '0'});
            $(this).text("Hide Structure");
       }
       else
       {
           $('#json-loader').css({left: '-490px'});
           $(this).text("Load Structure");
       } 
    });*/

    /*$('#structureTextArea').on('input', function(){
        $('svg').remove();

        var structure = $(this).val();
        generateTree(JSON.parse(structure));
    ...
    {
        data = JSON.parse(localStorage.structure);
    }

    if(data)
    {
        $('#structureTextArea').val(JSON.stringify(data, null, 2));
        generateTree(data);
    }*/
});